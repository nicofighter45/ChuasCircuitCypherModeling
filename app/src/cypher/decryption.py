...
class Decryption(Simulation):
    ...
    def vr(self, t, _=0, __=0):
        k = get_k_from_t(t)
        return self.encryptor.vr(t, self.encryptor.solution.y[0][k], self.encryptor.solution.y[1][k])
    ...
    def p_list(self):
        ...
        for t in self.times:
            k = get_k_from_t(t)
            p_list.append(self.p(t, self.solution.y[0][k], self.solution.y[1][k]))
        return np.array(p_list)

    def continuous_p(self, t):
        return self.p(t, self.solution.sol(t)[0], self.solution.sol(t)[1])
